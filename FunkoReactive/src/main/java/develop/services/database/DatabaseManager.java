...
/**
 * Clase que gestiona la conexion a la base de datos y proporciona un pool de conexiones.
 * Esta clase implementa el patron Singleton para garantizar una unica instancia en la aplicacion.
 */
public class DatabaseManager {
    ...
    private DatabaseManager() {
        ...
        ConnectionPoolConfiguration configuration = ConnectionPoolConfiguration
                ...
                .maxIdleTime(Duration.ofMillis(1000)) // Tiempo maximo de espera
                .maxSize(20) // Tamano maximo del pool
                .build();
    ...
    /**
     * Obtiene una instancia unica de DatabaseManager utilizando el patron Singleton.
     *
    ...
    private synchronized void loadProperties() {
        ...
        try {
            ...
            logger.error("Error al leer el fichero de configuracion de la base de datos " + e.getMessage());
        }
    }
    ...
    public Mono<Void> executeScript(String scriptSqlFile) {
        logger.debug("Ejecutando script de inicializacion de la base de datos: " + scriptSqlFile);
        return Mono.usingWhen(
                connectionFactory.create(),
                connection -> {
                    logger.debug("Creando conexion con la base de datos");
                    String scriptContent = null;
                    try {
                        try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream(scriptSqlFile)) {
                            if (inputStream == null) {
                                return Mono.error(new IOException("No se ha encontrado el fichero de script de inicializacion de la base de datos"));
                            } else {
